...
def assign_chores(
    ...
    for single_day in daterange(start_date, end_date):  # loop through days
        if single_day.dayofweek in [6, 0, 1, 2, 3]:  # if it is a workday
            ...
            crew_present = crew_dates[
                (crew_dates[start_col] < single_day)
                & (crew_dates[end_col] > single_day)
            ]
            for person in crew_present.index:
                crew_dates.loc[person, "workdays"] = (
                    crew_dates.loc[person, "workdays"] + 1
                )  # +1 to their workday count
    ...
    job_list = ["breakfast1", "breakfast2", "lunch1", "lunch2", "dinner1", "dinner2"]
    ...
    for day in workdays:
        present_df = crew_dates[
            (crew_dates[start_col] <= day) & (crew_dates[end_col] >= day)
        ]
        ...
        while bf_count < 2:
            bf_df = present_df[
                (present_df["breakfast_ct"] < present_df["breakfast_max"])
            ]
    ...
    crew_dates["assigned_jobs"] = (
        crew_dates["breakfast_ct"] + crew_dates["lunch_ct"] + crew_dates["dinner_ct"]
    )
