...
def get_measuring_progress(year, measure_col="Weight"):
    ...
    print(
        f"At {dt}, there are {round(pct_remain, 1)}% remaining ({not_done}/{done+not_done}) to be measured"
    )
    return dt, not_done
...
def get_classification_progress(year, bags_col="NumBags"):
    ...
    print(
        f"At {dt} there are {round(pct_remain, 1)}% of bags remaining ({bags_remain}/{pts_w_bags}) to be classified."
    )
